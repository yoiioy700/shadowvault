...
import { Abi, AbiEntry, AbiEnums, AbiStructs, CallData, parseCalldataField, createAbiParser } from "starknet";
    ...
    for (const member of clonedKeyMembers) {
        if (member.value !== undefined) {
            if (!member.type.startsWith("core::array::Array::") && Array.isArray(member.value)) {
                keys = keys.concat(
                    mergeArrays(
                        member.value.map((matchingItem: any) =>
                            serializeEventKey(matchingItem, member, structs, enums, abi).map((item) => [item]),
                        ),
                    ),
                );
                ...
                const serializedKeys = serializeEventKey(member.value, member, structs, enums, abi).map((item) => [
                    item,
                ]);
...
export function buildEventKeys(
    ...
    if (filters) {
        keys = keys.concat(composeEventFilterKeys(filters as any, eventAbi as any, abi));
    }
