...
export function isValidContractArgs(args: unknown, expectedLength: number): boolean {
    return (
        Array.isArray(args) &&
...
export function normalizeToHexAddress(input: string | bigint | number | null | undefined): `0x${string}` {
    if (!input) return "0x0";
...
export function formatStrk(value: any): string {
    try {
        if (value && typeof value === "object" && "Some" in value && "None" in value) {
            if (value.Some !== undefined) return (Number(BigInt(value.Some)) / 1e18).toFixed(4) + " STRK";
            return "0 STRK";
        }
        if (typeof value === "bigint") return (Number(value) / 1e18).toFixed(4) + " STRK";
        if (typeof value === "string") {
            if (value.trim() === "" || value === "0") return "0 STRK";
            return (Number(BigInt(value)) / 1e18).toFixed(4) + " STRK";
        }
        if (typeof value === "number") return value === 0 ? "0 STRK" : (value / 1e18).toFixed(4) + " STRK";
    } catch {}
    return "0 STRK";
}
