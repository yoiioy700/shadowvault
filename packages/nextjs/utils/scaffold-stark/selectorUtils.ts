...
const contractAbis = deepMergeContracts(deployedContracts, mergedPredeployedContracts);
 ...
 */
export function getAllFunctionNamesFromABI(targetNetworkName: string): string[] {
 ...
 */
export function getFunctionNameFromSelector(selector: string, targetNetworkName: string): string {
    // Check cache first
    ...
    try {
        ...
        for (const functionName of functionNames) {
            try {
                ...
                if (computedSelector.toLowerCase() === normalizedSelector.toLowerCase()) {
                    selectorCache.set(cacheKey, functionName);
                    return functionName;
                }
            } catch (error) {
                // Continue to next function name if hash computation fails
                console.warn(`Failed to compute hash for function ${functionName}:`, error);
            }
        }
