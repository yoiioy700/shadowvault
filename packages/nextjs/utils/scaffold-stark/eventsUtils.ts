import { Abi, CallData, RpcProvider, events as starknetEvents, createAbiParser } from "starknet";
...
>(abi: Abi | undefined, eventName: string) {
    const matches = (abi as Abi | undefined)?.filter(
        (part) => part.type === "event" && part.name.split("::").slice(-1)[0] === eventName,
    ) as ExtractAbiEvent<ContractAbi<TContractName>, TEventName>[] | undefined;
    if (!matches || matches.length === 0) return undefined;
    if (matches.length > 1) throw new Error(`Ambiguous event "${String(eventName)}"`);
    return matches[0];
}
...
export async function enrichLog(
    ...
    const [block, transaction, receipt] = await Promise.all([
        opts.block && log.block_hash !== null ? client.getBlockWithTxHashes(log.block_hash) : Promise.resolve(null),
...
export async function getLatestAcceptedBlockNumber(client: RpcProvider): Promise<bigint> {
