 ...
 */
export const getCairoType = (val: any, paramName?: string, argTypes?: Record<string, string>): string => {
 ...
 */
export const hasMeaningfulDecodedArgs = (parsedArgs: Record<string, any>): boolean => {
    return (
        Object.keys(parsedArgs).length > 0 &&
        !Object.keys(parsedArgs).every((key) => key.startsWith("key") || key.startsWith("data"))
    );
};
