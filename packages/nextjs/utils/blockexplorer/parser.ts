...
export const convertCalldataToReadable = (calldata: string[]): { to: string; selector: string; args: string[] }[] => {
    ...
    while (currentPointer < calldata.length) {
        ...
        const args = calldata.slice(currentPointer + 3, currentPointer + 3 + argsLength);
 ...
 */
export const findFunctionDefinition = (selector: string, mergedContracts: any, contractAddress?: string) => {
    try {
                    ...
                    if (abiEntry.type === "interface" && abiEntry.items) {
                        ...
                        for (const item of abiEntry.items) {
                            if (item.type === "function" && item.name) {
                                const functionSelector = num.toHex(hash.starknetKeccak(item.name));
 ...
 */
export const decodeFunctionArguments = (functionDefinition: any, args: string[]) => {
    try {
        const decodedArgs: Record<string, any> = {};
        const argTypes: Record<string, string> = {};

        if (!functionDefinition.inputs || !Array.isArray(functionDefinition.inputs)) {
        ...
        for (const input of functionDefinition.inputs) {
            ...
            try {
                ...
                if (type === "core::integer::u256") {
                    ...
                    try {
                        ...
                        if (startIdx + numFullWords < args.length) {
                            ...
                            const pendingLen = parseInt(args[startIdx + numFullWords + 1], 16);
                            if (pendingLen > 0) {
                ...
                } else if (type === "core::starknet::contract_address::ContractAddress") {
                    decodedArgs[name] = args[dataIndex];
                    dataIndex += 1;
                } else if (type === "core::bool") {
                    decodedArgs[name] = args[dataIndex] === "0x1" || args[dataIndex] === "1";
                    dataIndex += 1;
                } else {
...
export const checkSanitizedEquals = (a: string, b: string) => {
    return encode.sanitizeHex(a || "").toLowerCase() === encode.sanitizeHex(b || "").toLowerCase();
};
