...
export function addError(state: FormErrorMessageState, key: string, message: string): FormErrorMessageState {
    return {
        ...state,
        [key]: message,
    };
...
export function clearError(state: FormErrorMessageState, key: string): FormErrorMessageState {
    delete state[key];
    return state;
}
...
const getFieldType = (type: string, abi: Abi): any => {
    if (type.startsWith("core::array::Array") || /^\([^()]*\)$/.test(type) || baseType.has(type)) {
        return { type };
    }
    return abi.find((item) => item.name === type);
};
const _decodeContractResponseItem = (respItem: any, abiType: any, abi: Abi, showAsString?: boolean): any => {
    if (respItem === undefined) {
        return "";
    }
    if (abiType.type && baseNumberType.has(abiType.type)) {
        if (typeof respItem === "bigint") {
            if (respItem <= BigInt(Number.MAX_SAFE_INTEGER) && respItem >= BigInt(Number.MIN_SAFE_INTEGER)) {
    ...
    if (abiType.type && abiType.type === "core::starknet::contract_address::ContractAddress") {
    ...
    if (abiType.type && /^\([^()]*\)$/.test(abiType.type)) {
        ...
        try {
            const tupleMatch: RegExpMatchArray | null = abiType.type.match(/\(([^)]+)\)/);
            ...
            const decodedArr: any[] = tupleTypes.map((type: string, index: number) => {
    ...
    if (abiType.type === "struct") {
        ...
        for (const [structKey, structValue] of Object.entries(respItem)) {
            const structItemDef = (members || []).find((item: any) => item.name === structKey);
    ...
    if (abiType.type === "enum") {
        ...
        if (typeof respItem === "object" && respItem !== null) {
            if (respItem.variant) {
                ...
                const variants = Array.isArray(abiType.variants) ? abiType.variants : [];

                for (const [enumKey, enumValue] of Object.entries(variant)) {
                    if (enumValue === undefined) continue;

                    const enumItemDef = variants.find((item: any) => item.name === enumKey);
            ...
            if (enumKeys.length === 1) {
                ...
                const enumVariant = abiType.variants?.find((v: any) => v.name === enumKey);
                if (enumVariant) {
...
export const decodeContractResponse = ({
    ...
    const abiTypes = (functionOutputs || []).map((output) => output.type).map((type) => getFieldType(type, abi));
    let arrResp = [resp];
    ...
    if (asText) {
        return typeof decodedResult === "string" ? decodedResult : JSON.stringify(decodedResult, replacer);
    }
...
export const displayType = (type: string) => {
    ...
    // arrays and options
    else if (isCairoResult(type) || isCairoArray(type) || isCairoOption(type) || isCairoSpan(type)) {
