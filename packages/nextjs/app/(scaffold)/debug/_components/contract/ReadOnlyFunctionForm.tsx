...
export const ReadOnlyFunctionForm = ({ contractAddress, abiFunction, abi }: ReadOnlyFunctionFormProps) => {
    ...
    const inputIsValidArray = isValidContractArgs(inputValue, abiFunction.inputs.length);
    ...
    const handleRead = () => {
        ...
        if (!isValidInput) {
             ...
             */
            console.warn(`Read blocked: Expected ${expectedArgCount} args, got ${newInputValue.length}`);
            return;
        }
    ...
    return (
        <div className="flex flex-col gap-3 py-5 first:pt-0 last:pb-1">
            <p className="font-medium my-0 break-words text-function">{abiFunction.name}</p>
            {inputElements}
