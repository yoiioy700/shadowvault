...
import { addError, clearError, displayType, FormErrorMessageState } from "./utilsDisplay";
import {
    isCairoArray,
    isCairoBigInt,
    isCairoInt,
...
export const ContractInput = ({
    ...
    const inputProps = {
        ...
        placeholder: paramType.name ? `${displayType(paramType.type)} ${paramType.name}` : displayType(paramType.type),
        onChange: (value: any) => {
            setForm((form) => ({
                ...form,
    ...
    const renderInput = () => {
        ...
        } else if (isCairoInt(paramType.type) || isCairoBigInt(paramType.type) || isCairoU256(paramType.type)) {
            return (
                <IntegerInput
                    {...inputProps}
                    ...
                    onError={(errMessage: string | null) =>
                        setFormErrorMessage((prev) => {
                            if (!!errMessage) return addError(prev, "intError" + stateObjectKey, errMessage);
                            return clearError(prev, "intError" + stateObjectKey);
                        })
                    }
                />
            );
        } else if (isCairoType(paramType.type) && !isCairoResult(paramType.type) && !isCairoOption(paramType.type)) {
    ...
    return (
        <div className="flex flex-col gap-1.5 w-full">
            <div className="flex items-center ml-2">
                ...
                <span className="block text-xs font-extralight leading-none">{displayType(paramType.type)}</span>
            </div>
            {renderInput()}
        </div>
    );
