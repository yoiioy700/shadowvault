...
export const DisplayVariable = ({
    ...
    if (isPredeployedTokenNameOrSymbol) {
        const tokenInfo =
            TOKEN_INFO[ETH_ADDRESS.toLowerCase() === contractAddress.toLowerCase() ? ETH_ADDRESS : STRK_ADDRESS];
        const value = abiFunction.name === "name" ? tokenInfo.name : tokenInfo.symbol;
        ...
        return (
            <div className="space-y-1 pb-2">
                ...
                <div className="text-neutral font-medium flex flex-col items-start">
                    <div>
                        <div className="break-all block transition bg-transparent">{value}</div>
                    </div>
                </div>
            </div>
        );
