...
export default function HistoryModal({ entry, onClose }: { entry: HistoryEntry; onClose: () => void }) {
    const { theme } = useTheme();
    const isDarkMode = theme === "dark";
    const formatDate = (ts: number) => formatTimestamp(ts);

    const isSuccess = entry.status === "success";
    const chipClasses = isSuccess ? "bg-green-500/20 text-green-400" : "bg-red-500/20 text-red-400";
            ...
            >
                <div className="flex items-center justify-between mb-2 flex-shrink-0">
                    <div className="flex items-center gap-2">
                        ...
                        <span className={`inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded ${chipClasses}`}>
