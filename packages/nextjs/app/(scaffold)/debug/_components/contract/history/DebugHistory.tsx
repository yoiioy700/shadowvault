...
export default function DebugHistory() {
    const [selectedContract] = useLocalStorage<ContractName>("scaffoldStark2.selectedContract", contractNames[0], {
    ...
    const entries = useMemo(() => historyByContract[selectedAddress] || [], [historyByContract, selectedAddress]);
    ...
    return (
        <div className="h-full max-h-[650px] w-full xl:w-[400px] space-y-4">
            <div className="tab h-10 w-full xl:w-1/3 tab-active bg-[#8A45FC]! rounded-[5px] text-white!">History</div>
                ...
                <div className="flex flex-col">
                    {formatted.length === 0 ? (
                        ...
                        formatted.map((e, idx) => (
                            ...
                            >
                                <span className="truncate mr-2 text-[#4DB4FF]">{e.functionName}</span>
                                <div className="flex items-center gap-3">
                                    <span className="text-xs text-neutral">{formatDate(e.timestamp)}</span>
