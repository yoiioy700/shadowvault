...
import { AbiEnum, AbiFunction, AbiParameter, AbiStruct, parseTuple } from "~~/utils/scaffold-stark/contract";
/**
 * Generates a key based on function metadata
 */
export const getFunctionInputKey = (functionName: string, input: AbiParameter, inputIndex: number): string => {
...
export const getArgsAsStringInputFromForm = (form: Record<string, any>) => {
    const _encodeValueFromKey = (key: string = "", value: any): any => {
        ...
        if (isCairoArray(key)) {
            ...
            return value.map((arrayValue: any) => _encodeValueFromKey(genericType, arrayValue));
        }
        ...
        if (!key.includes("core::") || isCairoResult(key) || isCairoOption(key)) {
            ...
            if (Object.keys(structObject).includes("variant")) {
                ...
                // check for option
                if (enumVariants.includes("Some") && enumVariants.includes("None") && isCairoOption(key)) {
                ...
                // check for result
                if (enumVariants.includes("Ok") && enumVariants.includes("Err") && isCairoResult(key)) {
                    // for some value we return with the corresponding value
                    ...
                    else if (typeof (enumObject.Err as FormStructValue).value !== undefined) {
                        return new CairoResult(
        ...
        if (isCairoTuple(key)) {
            ...
            return cairo.tuple(
                ...tupleValues.map((tupleValue, index) => _encodeValueFromKey(tupleKeys[index], tupleValue)),
            );
        }
        ...
        if (isValidNumber(value) && (isCairoBigInt(key) || isCairoInt(key) || isCairoFelt(key) || isCairoU256(key))) {
            return num.toBigInt(value);
        }
