...
const DistributionPanel = () => {
  ...
  const isDeadBool = isDead === true || isDead === 1n || isDead === 1;
      ...
      <div className="card-body">
        ...
        {canDistribute && (
          <div className="card-actions justify-end mt-4">
            <button className={`btn btn-error btn-lg ${isDistributing ? "loading" : ""}`} onClick={handleDistribute} disabled={isDistributing}>
        ...
        {!isDeadBool && (
          <div className="alert alert-info mt-4">
            <span>Distribution can only be triggered when the vault owner is marked as dead (heartbeat expired).</span>
          </div>
        )}
      </div>
    </div>
