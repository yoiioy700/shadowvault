 ...
 */
export const Address = ({ address, disableAddressLink, format, profile, isLoading, size = "base" }: AddressProps) => {
    ...
    return (
        <div className="flex items-center">
            ...
            {disableAddressLink ? (
                <span className={`ml-1.5 text-${size} font-normal`}>{profile?.name || displayAddress}</span>
            ) : targetNetwork.network === devnet.network ? (
                <span className={`ml-1.5 text-${size} font-normal`}>
                    <Link href={blockExplorerAddressLink}>{profile?.name || displayAddress}</Link>
                </span>
            ) : (
                <a
