...
import { Avatar, BlockieAvatar, isStarknetName } from "~~/components/scaffold-stark";
...
export const AddressInfoDropdown = ({ address, displayName, blockExplorerAddressLink }: AddressInfoDropdownProps) => {
    ...
    const [, setWasDisconnectedManually] = useLocalStorage<boolean>("wasDisconnectedManually", false, {
        initializeWithValue: false,
    });
            ...
            <details ref={dropdownRef} className="dropdown dropdown-end leading-3">
                <summary className="btn bg-transparent btn-sm px-2 py-[0.35rem] dropdown-toggle gap-0 !h-auto border border-[#5c4fe5] ">
                    <div className="hidden [@media(min-width:412px)]:block">
                        <Avatar address={address} size={30} profilePicture={profile?.profilePicture} />
                    </div>
                ...
                >
                    ...
                    <li className={selectingNetwork ? "hidden" : ""}>
                        <label htmlFor="qrcode-modal" className="btn-sm !rounded-xl flex gap-3">
