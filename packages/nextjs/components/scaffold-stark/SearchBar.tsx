...
export const SearchBar: React.FC<SearchBarProps> = ({
    ...
    const determineInputType = async (input: string): Promise<"transaction" | "address" | "invalid"> => {
    ...
    const handleSearch = async () => {
        ...
        try {
            ...
            setError("An error occurred while validating the input. Please try again.");
            ...
            <div className="relative">
                <div className="relative">
                    <input
                        ...
                        className={`w-full px-4 py-3 pr-12 text-base bg-base-100 border rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent transition-colors ${
                            error ? "border-error focus:ring-error" : "border-base-300 hover:border-base-400"
                        }`}
                ...
                {!error && (
                    <div className="mt-2 text-xs text-base-content/60">
                        Enter a transaction hash or contract address starting with 0x. We&apos;ll automatically detect
                        the type.
                    </div>
                )}
            </div>
