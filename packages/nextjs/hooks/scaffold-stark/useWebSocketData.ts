...
export const useWebSocketData = ({ topic, params, enabled = true, onMessage }: UseWebSocketDataConfig) => {
    ...
    const start = useCallback(async () => {
        ...
        try {
            ...
            let sub: any;
            if (topic === "newHeads") {
                sub = await channel.subscribeNewHeads((params as SubscribeNewHeadsParams) || {});
                ...
                sub = await channel.subscribeTransactionStatus(params as SubscribeTransactionStatusParams);
            }
            ...
            if (elapsed < minConnecting) {
                connectTimeoutRef.current = setTimeout(setConnected, minConnecting - elapsed);
            } else {
                setConnected();
            }
