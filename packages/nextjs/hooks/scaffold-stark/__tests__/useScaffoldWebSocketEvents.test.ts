...
vi.mock("~~/utils/scaffold-stark/eventsUtils", async () => {
    const actual = await vi.importActual<any>("~~/utils/scaffold-stark/eventsUtils");
    return {
        ...actual,
        parseLogsArgs: vi.fn(() => ({})),
...
describe("useScaffoldWebSocketEvents", () => {
    ...
    it("sets error when channel unavailable", async () => {
        ...
        expect(result.current.error === null || result.current.error instanceof Error).toBeTruthy();
    });
});
