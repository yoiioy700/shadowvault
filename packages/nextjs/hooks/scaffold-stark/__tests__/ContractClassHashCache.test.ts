...
describe("ContractClassHashCache", () => {
    ...
    it("should cache class hash results", async () => {
        ...
        const result1 = await cache.getClassHash(mockProvider as ProviderInterface, address);

        // Second call should use cache
        const result2 = await cache.getClassHash(mockProvider as ProviderInterface, address);
    ...
    it("should handle provider errors", async () => {
        ...
        vi.mocked(mockProvider.getClassHashAt)!.mockRejectedValueOnce(new Error("API Error"));

        const result = await cache.getClassHash(mockProvider as ProviderInterface, address);
    ...
    it("should deduplicate concurrent requests", async () => {
        ...
        const promises = Array(3)
            .fill(null)
            .map(() => cache.getClassHash(mockProvider as ProviderInterface, address));
    ...
    it("should use different cache keys for different block identifiers", async () => {
        ...
        vi.mocked(mockProvider.getClassHashAt)!.mockResolvedValueOnce(hash1).mockResolvedValueOnce(hash2);

        const result1 = await cache.getClassHash(mockProvider as ProviderInterface, address, "pre_confirmed");

        const result2 = await cache.getClassHash(mockProvider as ProviderInterface, address, "latest");
    ...
    it("should handle multiple addresses independently", async () => {
        ...
        const result1 = await cache.getClassHash(mockProvider as ProviderInterface, address1);

        const result2 = await cache.getClassHash(mockProvider as ProviderInterface, address2);
