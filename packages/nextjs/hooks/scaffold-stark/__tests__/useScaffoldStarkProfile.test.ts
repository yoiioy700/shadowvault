...
import { fetchProfileFromApi, useScaffoldStarkProfile } from "../useScaffoldStarkProfile";
...
global.fetch = vi.fn((url) => {
    if (url.includes("addr_to_domain")) {
        ...
        return Promise.resolve(createMockResponse({ id: "123", domain: { domain: "Test User" } }));
...
vi.mock("~~/scaffold.config", async (importOriginal: () => Promise<{ default: any }>) => {
