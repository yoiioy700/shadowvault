...
describe("useScaffoldWatchContractEvent", () => {
    ...
    beforeEach(async () => {
        ...
        const { useScaffoldWebSocketEvents } = await import("../useScaffoldWebSocketEvents");
    ...
    it("calls onLogs when WS events arrive", async () => {
        const { useScaffoldWebSocketEvents } = await import("../useScaffoldWebSocketEvents");
        (useScaffoldWebSocketEvents as unknown as any).mockImplementationOnce(({ onEvent }: any) => {
            const eventAbi = {
