...
export const useDataTransaction = (blockNumber: number) => {
    ...
    const calculateAverageFee = useCallback(
        async (blockIdentifier: number): Promise<number> => {
            try {
                const blockTxHashes = await publicClient.getBlockWithTxHashes(blockIdentifier);
        ...
        try {
            ...
            if (prevBlockNumber >= 0) {
                ...
                if (currentBlock && prevBlock) {
                    ...
                    const timeDiffBetweenBlocks = currentBlock.timestamp - prevBlock.timestamp;

                    tps = timeDiffBetweenBlocks > 0 ? currentTxCount / timeDiffBetweenBlocks : null;
                }
            }
