            ...
            if ((fromBlock && blockNumber >= fromBlock) || blockNumber >= fromBlockUpdated) {
                ...
                const enriched = await Promise.all(
                    newestFirst.map(async (log) => {
                        const { block, transaction, receipt } = await enrichLog(publicClient, log, {
                            block: !!blockData,
    ...
    useEffect(() => {
        if (!wsError && wsEvents && wsEvents.length) {
            ...
            const incoming = wsEvents.filter((we: any) =>
                existing.every(
                    (e: any) =>
                        e.log.transaction_hash !== we.log.transaction_hash || e.log.event_index !== we.log.event_index,
                ),
            );
            if (incoming.length) {
    ...
    const eventHistoryData = useMemo(() => {
        if (deployedContractData) {
            return (events || []).map((event) => {
                const args = parseLogsArgs(deployedContractData.abi as Abi, fullName as string, [event.log]);
