...
import { ContractAbi, ContractName, UseScaffoldWebSocketEventsConfig } from "~~/utils/scaffold-stark/contract";
import { getSharedWebSocketChannel } from "~~/services/web3/websocket";
import { enrichLog, resolveEventAbi, parseLogsArgs } from "~~/utils/scaffold-stark/eventsUtils";
...
>({
    ...
    const eventAbi = useMemo(() => {
        return resolveEventAbi<TContractName, TEventName>(deployedContractData?.abi as Abi, eventName as string);
    ...
    const start = useCallback(async () => {
        ...
        setIsLoading(true);
        try {
            const channel: WebSocketChannel | null = await getSharedWebSocketChannel(targetNetwork);
            if (!channel) {
                throw new Error("WebSocket channel unavailable");
            }

            console.log("🔌 [useScaffoldWebSocketEvents] WebSocket channel connected successfully");
            ...
            const params: SubscribeEventsParams = {
                ...
                keys,
                blockIdentifier: typeof fromBlock !== "undefined" ? Number(fromBlock) : undefined,
            };
            ...
            sub.on(async (evt: any) => {
                console.log("📡 [useScaffoldWebSocketEvents] New event received via WebSocket:", evt);
                ...
                const { block, transaction, receipt } = await enrichLog(httpClient, evt, {
                    block: true,
                    transaction: true,
                    receipt: true,
                });
    ...
    }, [enabled, deployedContractData?.address, eventAbi, filters, fromBlock, targetNetwork]);
    ...
    }, [contractName, eventName, enabled, deployedContractData, eventAbi, deployedContractLoading]);

    const parsedEvents = useMemo(() => {
        if (!deployedContractData || !eventAbi) return [];
        return events.map((e) => {
            const args = parseLogsArgs(deployedContractData.abi as Abi, eventAbi.name, [e.log]);
