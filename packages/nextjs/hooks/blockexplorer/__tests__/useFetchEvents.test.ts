...
describe("useFetchEvents", () => {
    ...
    it("should fetch events for a specific transaction hash", async () => {
        ...
        const { result } = renderHook(() => useFetchEvents({ transactionHash: txHash }), {
            wrapper: createWrapper(),
        });
    ...
    it("should handle ABI-based event parsing with different data types", async () => {
        ...
        const { result } = renderHook(() => useFetchEvents({ address: "0x123456789" }), {
            wrapper: createWrapper(),
        });
    ...
    it("should handle events with ByteArray data", async () => {
        ...
        const { deepMergeContracts } = await vi.importMock("~~/utils/scaffold-stark/contract");
    ...
    it("should handle pagination correctly", async () => {
        ...
        const { result } = renderHook(() => useFetchEvents({ address: "0x123456789", pageSize: 10, page: 2 }), {
            wrapper: createWrapper(),
        });
    ...
    it("should handle block timestamp fetch errors gracefully", async () => {
        ...
        mockRpcProvider.getBlockWithTxHashes.mockRejectedValue(new Error("Block not found"));
    ...
    it("should handle transaction receipt errors for transaction hash queries", async () => {
        ...
        mockRpcProvider.getTransactionReceipt.mockRejectedValue(new Error("Transaction not found"));
    ...
    it("should handle address events fetch errors gracefully", async () => {
        ...
        mockRpcProvider.getEvents.mockRejectedValue(new Error("Events fetch failed"));
    ...
    it("should handle complete fetch errors", async () => {
        mockRpcProvider.getBlockLatestAccepted.mockRejectedValue(new Error("Network error"));
