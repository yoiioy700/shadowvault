...
vi.mock("../../utils/scaffold-stark/selectorUtils", () => ({
    getFunctionNameFromSelector: vi.fn((selector) => `Unknown Function (${selector})`),
}));

vi.mock("~~/utils/Constants", () => ({
    devnetUDCAddress: "0x041a78e741e5af2fec34b695679bc6891742439f7afb8484ecd7766661ad02bf",
    universalStrkAddress: "0x04718f5a0fc34cc1af16a1cdee98ffb20c31f5cd61d6ab07201858f4287c938d",
    strkAbi: [],
    devnetStrkClassHash: "0x046ded64ae2dead6448e247234bab192a9c483644395b66f2155f2614e5804b0",
    sepoliaMainnetStrkClassHash: "0x04ad3c1dc8413453db314497945b6903e1c766495a1e60492d44da9c2a986e4b",
}));
...
vi.mock("starknet", () => ({
    ...
    hash: {
        starknetKeccak: vi.fn((name) => {
            ...
            if (name === "transfer") return "0x83afd3f4caedc6eebf44246fe54e38c95e3179a5ec9ea81740eca5b482d12e";
...
describe("useFetchAllTxns", () => {
    ...
    it("should fetch transactions with default pagination", async () => {
        ...
        mockRpcProvider.getTransaction.mockResolvedValueOnce(mockTxInstance1).mockResolvedValueOnce(mockTxInstance2);
    ...
    it.skip("should detect UDC contract deployments", async () => {
        const udcAddress = "0x041a78e741e5af2fec34b695679bc6891742439f7afb8484ecd7766661ad02bf";
        const mockBlock = {
            timestamp: 1640995200,
            transactions: ["0xtx1"],
            block_hash: "0xblock1",
    ...
    it.skip("should filter transactions by sender address", async () => {
        ...
        mockRpcProvider.getBlock.mockResolvedValue(mockBlock);
        mockRpcProvider.getTransaction.mockResolvedValueOnce(mockTxInstance1).mockResolvedValueOnce(mockTxInstance2);
        mockRpcProvider.getTransactionReceipt.mockResolvedValueOnce(mockTxReceipt).mockResolvedValueOnce(mockTxReceipt);

        const { result } = renderHook(() => useFetchAllTxns({ bySenderAddress: "0xsender1" }), {
            wrapper: createWrapper(),
        });
    ...
    it("should filter transactions by receiver address", async () => {
        ...
        const { result } = renderHook(() => useFetchAllTxns({ byReceiverAddress: "0xcontract1" }), {
            wrapper: createWrapper(),
        });
    ...
    it.skip("should apply union filter when both sender and receiver filters are provided", async () => {
        ...
        expect(result.current.txns).toHaveLength(2);
        expect(result.current.txns.some((tx) => tx.fromAddress === "0xsender1")).toBe(true);
        expect(result.current.txns.some((tx) => tx.txCalls.some((call) => call.toAddress === "0xcontract2"))).toBe(
            true,
        );
    });
    ...
    it("should handle pagination correctly", async () => {
        ...
        mockRpcProvider.getBlock.mockResolvedValueOnce(mockBlocks[0]).mockResolvedValueOnce(mockBlocks[1]);
        mockRpcProvider.getTransaction.mockResolvedValue(mockTxInstance);
        mockRpcProvider.getTransactionReceipt.mockResolvedValue(mockTxReceipt);

        const { result } = renderHook(() => useFetchAllTxns({ page: 1, pageSize: 1 }), {
            wrapper: createWrapper(),
        });
    ...
    it("should handle transaction processing errors gracefully", async () => {
        ...
        mockRpcProvider.getBlock.mockResolvedValue(mockBlock);
        mockRpcProvider.getTransaction.mockRejectedValue(new Error("Transaction not found"));
