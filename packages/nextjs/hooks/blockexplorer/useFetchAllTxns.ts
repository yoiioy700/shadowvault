...
import { DECLARE_TXN_V3, DEPLOY_ACCOUNT_TXN_V3, INVOKE_TXN_V3, DEPLOY_TXN } from "@starknet-io/types-js";
...
import { checkSanitizedEquals, convertCalldataToReadable } from "~~/utils/blockexplorer";
...
export function useFetchAllTxns(options: UseFetchAllTxnsOptions = {}) {
    const { page = 1, pageSize = 7, bySenderAddress, byReceiverAddress } = options;
    ...
    const processBlockToTxns = useCallback(
        async (block: Awaited<ReturnType<typeof provider.getBlock>>) => {
            ...
            const { timestamp, transactions: txs, block_hash: blockHash, block_number: blockNumber } = block;
            ...
            for (const txHash of txs) {
                ...
                if (txInstance.type === "DECLARE") {
                    ...
                    explorerEntry.fromAddress = (txInstance as unknown as DECLARE_TXN_V3).sender_address;
                    ...
                    explorerEntry.fromAddress = (txInstance as unknown as DEPLOY_ACCOUNT_TXN_V3).contract_address_salt
                        ? `0x${(txInstance as unknown as DEPLOY_ACCOUNT_TXN_V3).contract_address_salt}`
                        : null;

                    txData.toAddress = (txInstance as unknown as DEPLOY_ACCOUNT_TXN_V3).contract_address_salt;
                    ...
                    explorerEntry.fromAddress = (txInstance as unknown as INVOKE_TXN_V3).sender_address;

                    const calls = convertCalldataToReadable((txInstance as unknown as INVOKE_TXN_V3).calldata);

                    // since devnet uses UDC and its treated as invoke, we record that case here
                    for (const call of calls) {
                        const _txData = { ...txData };

                        const isContractDeployment = checkSanitizedEquals(call.to, devnetUDCAddress);
                        ...
                        if (isContractDeployment) {
                            ...
                            _txData.functionCalled = getFunctionNameFromSelector(call.selector, targetNetwork.network);
                        }
                ...
                if ((bySenderAddress || "").toLowerCase() !== "" || (byReceiverAddress || "").toLowerCase() !== "") {
                    shouldIncludeTransaction = false;

                    // Check sender address filter
                    const matchesSender = !!bySenderAddress
                        ? checkSanitizedEquals(explorerEntry.fromAddress || "", bySenderAddress)
                        : true;
                    ...
                    const matchesReceiver = !!byReceiverAddress
                        ? txCalls.filter((call) => checkSanitizedEquals(call.toAddress || "", byReceiverAddress))
                              .length > 0
                        : true;
    ...
    const fetchPaginatedTxns = useCallback(async () => {
        ...
        const maxBlocksToProcess = Math.min(totalBlocks, Math.ceil((page * pageSize) / 2) + 10);
        const blocksToFetch: number[] = [];

        // Start from latest blocks and work backwards
        for (let i = totalBlocks - 1; i >= Math.max(0, totalBlocks - maxBlocksToProcess); i--) {
    ...
    }, [totalBlocks, provider, processBlockToTxns, page, pageSize, bySenderAddress, byReceiverAddress]);
