...
export const useFetchTxnDetail = (txHash?: string) => {
    ...
    const fetchTransactionDetail = async (): Promise<TransactionDetail | null> => {
        ...
        try {
            ...
            if (receiptData.execution_resources) {
                transactionDetail.executionResources = receiptData.execution_resources;
                transactionDetail.gasUsed = receiptData.execution_resources.steps?.toString();
                transactionDetail.l1GasConsumed = receiptData.execution_resources.l1_gas?.toString();
                transactionDetail.l2GasConsumed = receiptData.execution_resources.l2_gas?.toString();
            }
            ...
            switch (txInstance.type) {
                case "INVOKE":
                    ...
                    transactionDetail.maxFee = extractFeeAmount((invokeTxn as any).max_fee);
                    ...
                    if (invokeTxn.calldata && invokeTxn.calldata.length > 0) {
                        try {
                            ...
                            transactionDetail.functionCalls = calls.map((call) => {
                                ...
                                const functionInfo = findFunctionDefinition(call.selector, mergedContracts, call.to);
                                let decodedArgs: Record<string, any> = {};
                                let argTypes: Record<string, string> = {};

                                if (functionInfo) {
                                    const { decodedArgs: decoded, argTypes: types } = decodeFunctionArguments(
                                ...
                                return {
                                    contractAddress: call.to,
                                    entrypoint: getFunctionNameFromSelector(call.selector, targetNetwork.network),
                                    calldata: call.args,
                                    selector: call.selector,
                                    decodedArgs,
                                    argTypes,
                ...
                case "DECLARE":
                    ...
                    transactionDetail.maxFee = extractFeeAmount((declareTxn as any).max_fee);
                    break;

                case "DEPLOY_ACCOUNT":
                    const deployAccountTxn = txInstance as unknown as DEPLOY_ACCOUNT_TXN_V3;
                    transactionDetail.contractAddress = deployAccountTxn.contract_address_salt;
                    transactionDetail.signature = deployAccountTxn.signature;
                    transactionDetail.nonce = deployAccountTxn.nonce;
                    transactionDetail.maxFee = extractFeeAmount((deployAccountTxn as any).max_fee);
                    break;

                case "DEPLOY":
                    const deployTxn = txInstance as unknown as DEPLOY_TXN;
                    if (deployTxn.constructor_calldata && deployTxn.constructor_calldata.length > 0) {
                        transactionDetail.senderAddress = deployTxn.constructor_calldata[0];
                    }
                    break;
