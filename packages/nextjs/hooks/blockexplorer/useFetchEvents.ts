...
import { RpcProvider, hash, events as starknetEvents, CallData, num } from "starknet";
    ...
    try {
        ...
        for (const contractName in mergedContracts) {
            ...
            if (contract && contract.abi) {
                ...
                const eventDefinitions = contract.abi.filter((item: any) => item.type === "event");

                for (const eventDef of eventDefinitions) {
                    if (eventDef.name) {
                        // For nested events, we need to check both the full name and the short name
                        const fullEventNameHash = num.toHex(hash.starknetKeccak(eventDef.name));
                        const shortEventName = eventDef.name.split("::").pop() || "Event";
                        const shortEventNameHash = num.toHex(hash.starknetKeccak(shortEventName));

                        if (fullEventNameHash === selector || shortEventNameHash === selector) {
                            return {
                                eventDef,
                                contractName,
                    ...
                    if (eventDef.kind === "enum" && eventDef.variants) {
                        for (const variant of eventDef.variants) {
                            if (variant.kind === "nested" && variant.name) {
                                const variantHash = num.toHex(hash.starknetKeccak(variant.name));

                                if (variantHash === selector) {
                                    // Find the actual event definition
                                    const actualEventDef = eventDefinitions.find((ed: any) => ed.name === variant.type);
...
const decodeEventArguments = (eventDefinition: any, keys: string[], data: string[]) => {
    try {
 ...
 */
export function useFetchEvents(options: UseFetchEventsOptions = {}): UseFetchEventsResult {
    const { address, transactionHash, fromBlock = 0, toBlock, pageSize = 20, page = 1 } = options;
    ...
    const fetchEvents = useCallback(async (): Promise<{
        ...
        try {
            ...
            const latestBlock = toBlock || (await publicClient.getBlockLatestAccepted()).block_number;
            ...
            if (transactionHash) {
                ...
                try {
                    const receipt = await publicClient.getTransactionReceipt(transactionHash);
                    if ((receipt as any).events) {
                        const transaction = await publicClient.getTransactionByHash(transactionHash);
                        const block = await publicClient.getBlockWithTxHashes((receipt as any).block_hash);
                            ...
                            (receipt as any).events.map(async (event: any, index: number) => {
                                try {
                                    ...
                                    if (event.keys && event.keys.length > 0) {
                                        ...
                                        const eventInfo = findEventDefinition(selector, mergedContracts);
                        ...
                        events = await Promise.all(
                            paginatedEvents.map(async (event, index) => {
                                try {
                                    ...
                                    try {
                                        const block = await publicClient.getBlockWithTxHashes(event.block_hash);
                                        timestamp = block.timestamp;
                                    } catch (error) {
    ...
    }, [publicClient, address, transactionHash, fromBlock, toBlock, pageSize, page, targetNetwork]);
