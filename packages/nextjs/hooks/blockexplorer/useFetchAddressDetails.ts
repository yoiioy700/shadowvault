...
const getContractDeployerAndHash = async (address: Address, provider: RpcProvider) => {
    try {
        // Search for Deployed events from UDC that match our target address
        const events = await provider.getEvents({
            address: devnetUDCAddress,
            keys: [["0x26b160f10156dea0639bec90696772c640b9706a47f5b8c52ea1abe5858b34d"]], // Deployed event selector
            chunk_size: 1000,
        });
...
export const useFetchAddressDetails = (address?: Address | string) => {
                ...
                if (classHash && classHash !== "0x0") {
                    ...
                    try {
                        ...
                        if (contractAbi && typeof contractAbi === "object") {
                            const version = (await provider.getContractVersion(address)).cairo;
                    ...
                    const deploymentInfo = await getContractDeployerAndHash(address as Address, provider);
    ...
    const { data, isLoading, error, refetch } = useQuery({
        queryKey: ["address-details", address, targetNetwork.rpcUrls.public.http[0]],
