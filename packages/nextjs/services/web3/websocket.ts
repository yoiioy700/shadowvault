...
export const getWsUrlForChain = (chain: Chain): string => {
    ...
    switch (network) {
        case "devnet":
            return devnetWs || httpToWs(chain.rpcUrls.public.http[0] || "http://127.0.0.1:5050/rpc");
        case "sepolia":
            return (
                sepoliaWs ||
                httpToWs(chain.rpcUrls.public.http[0] || "https://starknet-sepolia.public.blastapi.io/rpc/v0_9")
            );
        case "mainnet":
            return (
                mainnetWs ||
                httpToWs(chain.rpcUrls.public.http[0] || "https://starknet-mainnet.public.blastapi.io/rpc/v0_9")
            );
        default:
...
export const getSharedWebSocketChannel = async (chain?: Chain): Promise<WebSocketChannel | null> => {
