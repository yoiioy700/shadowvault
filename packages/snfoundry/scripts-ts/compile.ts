 ...
 */
function needsRecompilation(srcDir: string, targetDevDir: string, verbose: boolean): boolean {
    ...
    for (const cairoFile of cairoFiles) {
        ...
        const expectedNames =
            moduleNames.length > 0
                ? moduleNames.map((m) => `${scarbInfo.packageName}_${m}.contract_class.json`)
        ...
        for (const expectedJsonName of expectedNames) {
            ...
            if (cairoFile.mtime > targetFile.mtime) {
                if (verbose) {
                    console.log(`📅 ${cairoFile.relativePath} is newer than ${expectedJsonName}`);
 ...
 */
function readScarbToml(srcDir: string): { packageName: string; mtime: number } | null {
    try {
...
function extractContractModuleNames(filePath: string): string[] {
    try {
        ...
        const names: string[] = [];
        const regex = /#\s*\[\s*starknet::contract\s*\][\s\S]{0,300}?pub\s+mod\s+([A-Za-z0-9_]+)/g;
