...
> => {
    const allContractsData: Record<string, Record<string, { address: string; abi: Abi; classHash: string }>> = {};
    ...
    const getPackageName = (): string => {
        ...
        try {
            ...
            const packageNameMatch = tomlContent.match(/\[package\][\s\S]*?name\s*=\s*"([^"]+)"/);
        ...
        if (path.extname(file) === ".json" && file.endsWith("_latest.json")) {
            ...
            Object.entries(content).forEach(([contractName, contractData]) => {
                try {
                    ...
                    const abiContent: CompiledSierra = JSON.parse(fs.readFileSync(abiFilePath, "utf8"));
...
const generateTsAbis = async () => {
    ...
    fs.writeFileSync(path.join(TARGET_DIR, "deployedContracts.ts"), formattedContent);
