    ...
    try {
        ...
        if (constructorAbi) {
            ...
            for (const arg of requiredArgs) {
                if (arg.type === "core::starknet::contract_address::ContractAddress" && constructorArgs[arg.name]) {
        ...
        return {
            ...
            error: `${userFriendlyMessage}${
                originalError !== userFriendlyMessage ? ` (Details: ${originalError})` : ""
            }`,
        };
    }
};
...
const estimateTip = async (): Promise<bigint> => {
    return networkName === "devnet" ? 1000n : (await deployer.getEstimateTip()).recommendedTip;
};

/**
 * Calculate estimated tip for declaration transaction with fee escalation
 */
const estimateDeclareFee = async (payload: DeclareContractPayload, compiledClassHash: string): Promise<bigint> => {
...
const getContractSize = (payload: DeclareContractPayload): number => {
    return typeof payload.contract === "string" ? payload.contract.length : JSON.stringify(payload.contract).length;
};
...
const declareIfNot_NotWait = async (payload: DeclareContractPayload, options?: UniversalDetails) => {
    ...
    try {
        ...
        console.log(green("Skipping declare - class hash"), classHash, green("already exists on-chain."));
        ...
        if (e instanceof RpcError && e.isType("CLASS_HASH_NOT_FOUND")) {
            console.log(yellow("Class hash"), classHash, yellow("not found, proceeding with declaration..."));
        } else {
    ...
    try {
        ...
        if (e instanceof RpcError && e.isType("VALIDATION_FAILURE") && e.baseError.data.includes("exceed balance")) {
            console.error(red("❌ Class declaration failed: Insufficient balance"));
            console.error(red(`   Deployer address: ${deployer.address}`));
            console.error(
                red(`   Please ensure your account has enough ${feeToken[0].name} to cover the declaration fee.`),
            );
            throw "Class declaration failed: insufficient balance";
        }

        if (e instanceof RpcError && (e.message?.includes("connection") || e.message?.includes("network"))) {
            ...
            console.error(red("   Please check your RPC URL and network connectivity."));
            throw "Class declaration failed: RPC connection error";
        }

        if (e instanceof RpcError && (e.message?.includes("timeout") || e.message?.includes("TIMEOUT"))) {
            console.error(red("❌ Class declaration failed: Request timeout"));
            console.error(red("   The RPC request timed out. Please try again or check your network connection."));
...
const deployContract_NotWait = async (payload: { salt: string; classHash: string; constructorCalldata: RawArgs }) => {
    try {
    ...
    if (constructorAbi) {
        ...
        for (const arg of requiredArgs) {
            if (
                ...
                throw new Error(red(`Missing value for constructor argument '${arg.name}' of type '${arg.type}'.`));
            }
        }

        const validationResult = validateConstructorArgsWithStarknetJS(abi, constructorArgs);
        if (!validationResult.isValid) {
            throw new Error(red(`Constructor validation failed: ${validationResult.error}`));
        }
    }

    const contractCalldata = new CallData(compiledContractSierra.abi);
    const constructorCalldata = constructorArgs ? contractCalldata.compile("constructor", constructorArgs) : [];
...
const executeDeployCalls = async (options?: UniversalDetails) => {
    ...
    try {
        ...
        let { transaction_hash } = await deployer.execute(deployCalls, executeOptions);
        ...
        if (e instanceof RpcError && e.isType("VALIDATION_FAILURE") && e.baseError.data.includes("exceed balance")) {
            console.error(red("❌ Deployment execution failed: Insufficient balance"));
            console.error(red(`   Deployer address: ${deployer.address}`));
            console.error(
                red(`   Please ensure your account has enough ${feeToken[0].name} to cover the deployment fees.`),
            );
            throw "Deployment tx execution failed: insufficient balance";
        }

        if (e instanceof RpcError && (e.message?.includes("connection") || e.message?.includes("network"))) {
            console.error(red("❌ Deployment execution failed: RPC connection error"));
            console.error(red(`   Unable to connect to ${networkName} network.`));
            console.error(red("   Please check your RPC URL and network connectivity."));
